...
# ENDPOINT REMOVIDO POR SEGURANÇA - Admin já criado
...
# ==================== ADS INTEGRATIONS (Meta & Google) ====================

class AdsConfig(BaseModel):
    provider: str  # meta, google
    account_id: Optional[str] = None
    access_token: Optional[str] = None
    refresh_token: Optional[str] = None

@api_router.get("/brands/{brand_id}/ads/providers")
async def get_ads_providers(brand_id: str, user: dict = Depends(get_current_user)):
    """Get available ads providers and connection status"""
...
@api_router.delete("/brands/{brand_id}/ads/{provider}")
...
@api_router.post("/brands/{brand_id}/ads/{provider}/sync")
async def sync_ads_data(brand_id: str, provider: str, user: dict = Depends(get_current_user)):
    """Sync ads data (MOCK - in production would call real APIs)"""
    ...
    return {"success": True, "synced_days": 30, "note": "MOCK - Dados simulados para demonstração"}

@api_router.get("/brands/{brand_id}/ads/{provider}/metrics")
async def get_ads_metrics(brand_id: str, provider: str, days: int = 30, user: dict = Depends(get_current_user)):
    """Get ads metrics for a provider"""
    data = await db.ads_metrics.find_one(
        {"brand_id": brand_id, "provider": provider},
        {"_id": 0}
    )
...
@api_router.get("/brands/{brand_id}/intelligence/summary")
async def get_intelligence_summary(brand_id: str, user: dict = Depends(get_current_user)):
    """Get unified intelligence summary from all sources"""
    brand = await db.brands.find_one({"brand_id": brand_id}, {"_id": 0})
    
    # Get Google Analytics data if connected
    ga_data = None
    ...
    brand_completeness = int((pillars_filled / len(pillar_keys)) * 100)
    
    return {
        "brand_health": {
            "completeness": brand_completeness,
...
async def generate_report(brand_id: str, report_type: str, format: str = "pdf", user: dict = Depends(get_current_user)):
    # Exclude 'history' from this route - it has a dedicated endpoint at /reports/history
...
async def get_admin_stats(user: dict = Depends(get_admin_user)):
    ...
    credits_pipeline = [
        {"$group": {
            "_id": None,
            "total_credits_sold": {"$sum": "$total_credits"},
            "total_credits_used": {"$sum": "$used_credits"},
            "total_credits_available": {"$sum": "$available_credits"}
        }}
    ]
    ...
    top_users_pipeline = [
        {"$group": {
            "_id": "$user_id",
            "total_calls": {"$sum": 1},
            "total_credits_used": {"$sum": {"$abs": "$credits"}}
        }},
        {"$sort": {"total_credits_used": -1}},
        {"$limit": 10}
    ]
    ...
    return {
        ...
        "credits": {
            "total_sold": credits_stats.get("total_credits_sold", 0),
            "total_used": credits_stats.get("total_credits_used", 0),
            "total_available": credits_stats.get("total_credits_available", 0)
        },
        "ai_usage": {
        ...
        "revenue": {
            "total": revenue_stats.get("total_revenue", 0),
            "transactions": revenue_stats.get("total_transactions", 0)
        },
...
async def get_admin_users(skip: int = 0, limit: int = 50, user: dict = Depends(get_admin_user)):
    ...
    # Enrich with credits info
    for u in users:
        credits = await db.ai_credits.find_one({"user_id": u["user_id"]}, {"_id": 0})
        u["credits"] = credits or {"total_credits": 0, "used_credits": 0, "available_credits": 0}
    
    return {"users": users, "total": total}

@api_router.get("/admin/ai-usage")
async def get_admin_ai_usage(days: int = 30, user: dict = Depends(get_admin_user)):
    """Get detailed AI usage stats"""
    from datetime import timedelta
    
    start_date = (datetime.now(timezone.utc) - timedelta(days=days)).isoformat()
    
    # All AI calls in period
    calls = await db.ai_credits_history.find(
    ...
    for call in calls:
        ...
        by_action[action]["credits"] += abs(call.get("credits", 0))
    
    # Estimate cost (rough estimate: $0.001 per credit)
    total_credits = sum(abs(c.get("credits", 0)) for c in calls)
    estimated_cost_usd = total_credits * 0.001
    ...
    return {
        ...
        "estimated_cost_brl": round(estimated_cost_usd * 5.5, 2),  # Rough USD to BRL
        "by_action": by_action,
...
CRM_PROVIDERS = {
    ...
    "kommo": {
        ...
        "api_base": "https://api.kommo.com/api/v4",
...
class CRMConfig(BaseModel):
    ...
    subdomain: Optional[str] = None  # Para Kommo

class CRMImportRequest(BaseModel):
    provider: str
    import_type: str = "contacts"  # contacts, leads, deals

@api_router.get("/crm/providers")
async def get_crm_providers():
    """List available CRM providers"""
    return {"providers": [
        {"id": k, "name": v["name"], "scopes": v["scopes"]} 
        for k, v in CRM_PROVIDERS.items()
    ]}

@api_router.get("/brands/{brand_id}/crm")
async def get_crm_integrations(brand_id: str, user: dict = Depends(get_current_user)):
    ...
    for provider in ["rdstation", "hubspot", "kommo"]:
        count = await db.crm_contacts.count_documents({
            "brand_id": brand_id, "source": provider
        })
        stats[provider] = {"contacts": count}
    
    return {"integrations": integrations, "stats": stats}

@api_router.post("/brands/{brand_id}/crm/connect")
async def connect_crm(brand_id: str, config: CRMConfig, user: dict = Depends(get_current_user)):
    ...
    integration_doc = {
        "brand_id": brand_id,
        "provider": config.provider,
        "provider_name": CRM_PROVIDERS[config.provider]["name"],
...
@api_router.delete("/brands/{brand_id}/crm/{provider}")
...
@api_router.post("/brands/{brand_id}/crm/import")
async def import_crm_data(brand_id: str, request: CRMImportRequest, user: dict = Depends(get_current_user)):
    """Import data from CRM (mock for now - real API calls would go here)"""
    ...
    # Mock import - in production, this would call the actual CRM API
    mock_contacts = [
    ...
    for contact in mock_contacts:
        ...
        await db.crm_contacts.update_one(
            {"brand_id": brand_id, "email": contact["email"], "source": request.provider},
            {"$set": contact_doc},
            upsert=True
        )
    ...
    return {
        "success": True, 
        "imported": len(mock_contacts),
        "message": f"{len(mock_contacts)} contatos importados do {CRM_PROVIDERS[request.provider]['name']}"
    }

@api_router.get("/brands/{brand_id}/crm/contacts")
...
# ==================== 6 NOVAS FEATURES: ALTO IMPACTO + DIFERENCIAÇÃO ====================

# --- 1. BRAND SCORE UNIFICADO ---
@api_router.get("/brands/{brand_id}/brand-score")
async def get_unified_brand_score(brand_id: str, user: dict = Depends(get_current_user)):
    """Calcula score unificado da marca baseado em múltiplas dimensões"""
    brand = await db.brands.find_one({"brand_id": brand_id}, {"_id": 0})
    if not brand:
        raise HTTPException(status_code=404, detail="Marca não encontrada")
    
    # Coletar scores de diferentes módulos
    ...
    pillar_score = min(100, pillar_count * 14)  # 7 pilares = 100%
...
async def generate_executive_pdf(brand_id: str, request: PDFReportRequest, user: dict = Depends(get_current_user)):
    ...
    # Estruturar o relatório (simulado - em produção geraria PDF real)
    report_data = {
        ...
        "pillar_summary": [
            {"name": p.get("pillar_type", ""), "status": "complete" if p.get("answers") else "pending"}
            for p in pillars
        ],
        "maturity_level": maturity.get("level", "Em análise") if maturity else "Não avaliado",
        "download_ready": True,
        "message": "Relatório PDF executivo gerado com sucesso! (MOCK - integração real pendente)"
    }
...
@api_router.get("/brands/{brand_id}/reports/history")
...
@api_router.post("/brands/{brand_id}/alerts/config")
async def save_alert_config(brand_id: str, config: EmailAlertConfig, user: dict = Depends(get_current_user)):
    ...
    doc = {
        ...
        "recipients": config.recipients if config.recipients else [user.get("email", "")],
        "updated_at": datetime.now(timezone.utc).isoformat(),
        "updated_by": user["user_id"]
    }
    await db.email_alerts_config.update_one(
        {"brand_id": brand_id}, {"$set": doc}, upsert=True
    )
    return {"success": True, "message": "Configuração de alertas salva!", "config": doc}

@api_router.post("/brands/{brand_id}/alerts/send-test")
async def send_test_alert(brand_id: str, user: dict = Depends(get_current_user)):
    """Envia email de teste (MOCK)"""
    brand = await db.brands.find_one({"brand_id": brand_id}, {"_id": 0})
    return {
        "success": True,
        "message": f"Email de teste enviado para {user.get('email', 'N/A')}! (MOCK)",
        "brand_name": brand.get("name", "N/A") if brand else "N/A",
        "preview": {
            "subject": f"[LaBrand] Relatório Semanal - {brand.get('name', 'Sua Marca') if brand else 'Sua Marca'}",
            "body_preview": "Seu score de marca está em 72 pontos. 2 novos alertas de consistência detectados..."
        }
    }

# --- 4. SOCIAL LISTENING LIGHT ---
@api_router.get("/brands/{brand_id}/social-listening/mentions")
async def get_social_mentions(brand_id: str, days: int = 30, user: dict = Depends(get_current_user)):
    """Busca menções da marca em redes sociais (MOCK)"""
    brand = await db.brands.find_one({"brand_id": brand_id}, {"_id": 0})
    brand_name = brand.get("name", "Marca") if brand else "Marca"
    
    # Dados simulados de social listening
    mock_mentions = [
        {"source": "Twitter", "text": f"Acabei de conhecer a {brand_name}, adorei o conceito!", "sentiment": "positive", "date": "2025-12-18", "reach": 1250},
        {"source": "Instagram", "text": f"@{brand_name.lower().replace(' ','')} muito bom atendimento 👏", "sentiment": "positive", "date": "2025-12-17", "reach": 3400},
        {"source": "LinkedIn", "text": f"A {brand_name} está revolucionando o mercado", "sentiment": "positive", "date": "2025-12-16", "reach": 890},
        {"source": "Twitter", "text": f"Tive um problema com {brand_name}, mas resolveram rápido", "sentiment": "neutral", "date": "2025-12-15", "reach": 450},
        {"source": "Facebook", "text": f"Recomendo a {brand_name} para todos!", "sentiment": "positive", "date": "2025-12-14", "reach": 2100},
    ]
    
    sentiment_summary = {"positive": 4, "neutral": 1, "negative": 0}
    ...
    return {
        ...
        "reach_total": sum(m["reach"] for m in mock_mentions),
        "mentions": mock_mentions,
        "top_sources": [
            {"source": "Instagram", "count": 1, "sentiment_avg": 90},
            {"source": "Twitter", "count": 2, "sentiment_avg": 75},
            {"source": "LinkedIn", "count": 1, "sentiment_avg": 85},
            {"source": "Facebook", "count": 1, "sentiment_avg": 90}
        ],
        "trending_topics": ["atendimento", "inovação", "qualidade"],
        "note": "MOCK - Integração real com APIs de social listening pendente"
    }
...
@api_router.post("/brands/{brand_id}/competitors/analyze-ai")
async def analyze_competitors_ai(brand_id: str, request: CompetitorAnalysisRequest, user: dict = Depends(get_current_user)):
    ...
    if balance < cost:
        raise HTTPException(status_code=402, detail=f"Créditos insuficientes. Necessário: {cost}, Disponível: {balance}")
    ...
    analysis_results = []
    for comp in request.competitors[:5]:  # Max 5 concorrentes
        analysis_results.append({
            "competitor": comp,
            "positioning": f"{comp} foca em preço competitivo e conveniência",
            "strengths": ["Preço acessível", "Distribuição ampla"],
            "weaknesses": ["Baixa diferenciação", "Atendimento genérico"],
            "opportunity_gaps": ["Experiência premium", "Personalização"],
            "threat_level": "medium"
        })
    ...
    result = {
        ...
        "strategic_recommendations": [
            "Diferenciar pela experiência do cliente",
            "Investir em branding emocional",
            "Explorar nichos não atendidos"
        ],
...
@api_router.get("/brands/{brand_id}/competitors/analyses")
async def get_competitor_analyses(brand_id: str, user: dict = Depends(get_current_user)):
    """Lista análises de concorrentes anteriores"""
    analyses = await db.competitor_analyses.find(
        {"brand_id": brand_id}, {"_id": 0}
...
class ContentGeneratorRequest(BaseModel):
    content_type: str  # tagline, post_social, bio, manifesto, elevator_pitch
...
async def generate_brand_content(brand_id: str, request: ContentGeneratorRequest, user: dict = Depends(get_current_user)):
    ...
    # Conteúdo simulado por tipo (em produção usaria LLM)
    generated = {
        "tagline": [
            f"{brand_name}: Transformando ideias em realidade",
            f"Inovação com propósito. Isso é {brand_name}.",
            f"{brand_name} - Onde a excelência encontra a paixão"
        ],
        "post_social": [
            f"🚀 Na {brand_name}, acreditamos que cada desafio é uma oportunidade de crescer. Como você está transformando seus obstáculos hoje? #Inovação #{brand_name.replace(' ','')}",
            f"✨ O que nos move? A busca constante pela excelência. {brand_name} - comprometidos com seu sucesso. #Qualidade #Compromisso"
        ],
        "bio": [
            f"{brand_name} é uma empresa dedicada a entregar soluções inovadoras que transformam negócios. Com foco em qualidade e compromisso com resultados, ajudamos nossos clientes a alcançar seus objetivos de forma sustentável."
        ],
        "manifesto": [
            f"Nós somos {brand_name}.\n\nAcreditamos que o futuro pertence àqueles que ousam sonhar grande e agir com propósito. Cada dia é uma nova oportunidade de fazer a diferença.\n\nNossa missão vai além dos números - buscamos impactar positivamente a vida das pessoas e das comunidades que servimos.\n\nCom integridade como bússola e inovação como combustível, construímos pontes entre o presente e o futuro que desejamos criar."
        ],
        "elevator_pitch": [
            f"Somos a {brand_name}, especialistas em [setor]. Ajudamos empresas a [benefício principal] através de [metodologia/produto único]. Nossos clientes experimentam [resultado mensurável]. Em um mercado onde [desafio comum], nos destacamos por [diferencial competitivo]."
        ]
    }
    
    suggestions = generated.get(request.content_type, ["Conteúdo não disponível"])
...
@api_router.get("/brands/{brand_id}/content-generator/history")
async def get_content_history(brand_id: str, user: dict = Depends(get_current_user)):
    """Lista conteúdos gerados anteriormente"""
    contents = await db.generated_content.find(
        {"brand_id": brand_id}, {"_id": 0}
    ).sort("generated_at", -1).to_list(50)
