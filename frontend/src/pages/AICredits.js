...
import { Zap, History, AlertTriangle, Loader2, Sparkles, CreditCard, CheckCircle } from 'lucide-react';
...
const ACTION_LABELS = {
  ...
  naming_generation: 'Geração de Naming',
  naming_phonetic: 'Análise Fonética',
  naming_global_friction: 'Fricção Global'
};
...
export default function AICredits() {
  ...
  const pollPaymentStatus = useCallback(async (sessionId, attempt = 0) => {
    ...
    if (attempt >= maxAttempts) {
      toast.info('Verifique seu email para confirmação do pagamento');
      return;
    }

    try {
      const response = await axios.get(
        `${API}/ai-credits/checkout-status/${sessionId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );

      if (response.data.payment_status === 'paid' || response.data.fulfilled) {
  ...
  }, [loadCreditsData, pollPaymentStatus]);
  ...
  const purchaseCredits = async (packageId) => {
    ...
    try {
      ...
      const msg = error.response?.data?.detail || 'Erro ao iniciar compra';
      toast.error(msg);
    } finally {
      setPurchasing(null);
    }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64" data-testid="credits-loading">
  ...
  return (
    <div className="space-y-6" data-testid="ai-credits-page">
      ...
      {paymentSuccess && (
        <div className="p-4 bg-green-50 border border-green-200 rounded-lg flex items-center gap-3" data-testid="payment-success-banner">
          <CheckCircle className="h-5 w-5 text-green-600" />
          <span className="text-green-700 font-medium">Pagamento confirmado! Seus créditos foram adicionados.</span>
        </div>
      )}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex items-baseline gap-2 mt-1">
                <span className="text-4xl font-bold" data-testid="credits-available">{creditsData?.available_credits || 0}</span>
      ...
      <div>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {CREDIT_PACKAGES.map((pkg) => (
            <Card key={pkg.id} className={`relative ${pkg.popular ? 'border-violet-500 ring-2 ring-violet-200' : ''}`} data-testid={`package-${pkg.id}`}>
              {pkg.popular && (
          ...
          {history.length > 0 ? (
            <div className="space-y-3">
              {history.slice(0, 10).map((item, i) => (
                <div key={i} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg" data-testid={`history-item-${i}`}>
                  <div>
